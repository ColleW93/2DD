<?php
	...
	if(count($_POST)>0) {
		$dbc = mysqli_connect('localhost', 'root', '', '2DD');
		$result = mysqli_query($dbc, "SELECT * FROM users WHERE email='" . $_POST["email"] . "' and password = '". $_POST["password"]."'");
		...
		<div class="container">
			<div class="row">
				<div class="col-sm-4 col-sm-offset-1">
					<div class="login-form" <?php if($message!="") { echo $message; } ?> ><!--login form-->
						<h2>Log Ind</h2>
						<form method="post" action="">
							...
							<span>
								...
								Lad mig forblive logget ind
							</span>
							<button type="submit" name="submit" value="Submit" class="btn btn-default">Log ind</button>
						</form>
					</div><!--/login form-->
				</div>
				<div class="col-sm-1">
				...
				<div class="col-sm-4">
					<div class="signup-form"><!--sign up form-->
						<h2>Ny Bruger</h2>
						<form action="">
							<input type="text" pattern="^[a-zA-Z]+$" placeholder="Navn"/>
							<input type="text" pattern="^[a-zA-Z ]+[0-9, ]+[a-zA-Z]+$" placeholder="Addresse 1, By"/>
							<input type="text" pattern="^[0-9]{8}$" placeholder="Tlf."/>
							<input type="email" pattern="^[a-zd._-]+@([a-zd-]+.)+[a-z]{2,6}$" placeholder="E-mail"/>
							<input type="password" patter="" placeholder="Kodeord"/>
