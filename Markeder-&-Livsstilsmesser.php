...
<head>
	...
	<link href="MogM.css" rel="stylesheet">
					...
					<div id="maingreet">
						<div class="carousel-inner">
							<div class="item active">
								<div class="col-sm-9">
									<h1><Span>Markeder & Livsstilsmesser</Span></h1>
									<p>Se her hvor vi kan findes på de forskellige markeder og livsstilsmesser</p>


									<?php
										$queryF = mysqli_query($dbc, "SELECT * FROM ml WHERE Season = 'F'");
										$queryS = mysqli_query($dbc, "SELECT * FROM ml WHERE Season = 'S'");
										$queryE = mysqli_query($dbc, "SELECT * FROM ml WHERE Season = 'E'");
										$queryV = mysqli_query($dbc, "SELECT * FROM ml WHERE Season = 'V'");
												
									?>

									<h1><Span id="sæson">Forår</Span></h1>
								 
						            <?php
						            if ( $queryF ) // valid result
									{
									    if ( mysqli_num_rows( $queryF ) > 0 ) // more than 0 records
									    	{
									        	while ($row = mysqlI_fetch_array($queryF)) 
							            	{
							            	?>  
								   			<div class="col-md-12" id="mlouterbox">  
					                            <div class="mlbox" >  
				                       	        <br />  
				                                <h4 class="titel"><?php echo $row["Titel"]; ?></h4>  
				                                <p class="dato">Start Dato: <?php echo $row["DatoStart"]; ?></p>
				                                <p class="dato">Slut Dato: <?php echo $row["DatoSlut"]; ?></p> 
				                                <p class="beskrivelse"><?php echo $row["Beskrivelse"]; ?></p>  
				                            	</div> 
						                	</div>			                
					                	<?php
					                		}
									    ...
									    {
									        echo '<p id="nulBegivenheder" style="font-size: 16px; color: black;">Ingen begivenheder.</p>';
									    }
									}
						            ?>  
									...
									<h1><Span id="sæson">Sommer</Span></h1>

									<?php
						            if ( $queryS ) // valid result
									{
									    if ( mysqli_num_rows( $queryS ) > 0 ) // more than 0 records
									    	{
									...
									<h1><Span id="sæson">Efterår</Span></h1>

									<?php
						            if ( $queryE ) // valid result
									{
									    if ( mysqli_num_rows( $queryE ) > 0 ) // more than 0 records
									    	{
									...
									<h1><Span id="sæson">Vinter</Span></h1>

									<?php
						            if ( $queryV ) // valid result
									{
									    if ( mysqli_num_rows( $queryV ) > 0 ) // more than 0 records
									    	{
									        	while ($row = mysqlI_fetch_array($queryV)) 
							            	{
							            	?>  
								   			<div class="col-md-12" id="mlouterbox">  
					                            <div class="mlbox" >  
									    ...
									    {
									        echo '<p id="nulBegivenheder" style="font-size: 16px; color: black; padding-bottom: 40px;">Ingen begivenheder.</p>';
									    }
									}
						            ?>  
